...
class Labelme2YOLO(object):
    ...
    def _save_yolo_image(self, json_data, json_name, image_dir_path, target_dir):
        ...
        # This line contains a bug! The file is already assumed to exist in the "YOLODataset" folder
        # But, this is impossible, so the imageData is read instead, but not all images have imageData!
        img_path = os.path.join(image_dir_path, target_dir,img_name)

        import shutil
        if not os.path.exists(img_path):

            # We know that "our image", or the image within the current sub-directory always exists
            src_img_path = str(self._json_dir) + "/" + img_name

            # Then just copy over our source image to the expected image or "YOLODataset" path
